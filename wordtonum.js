...
export function parse(str){
    ...
    if(str.startsWith("minus")) return -parse(str.slice(-(str.length - 5)).trim());
        ...
        if(!(t.includes(tens))) throw new SyntaxError(`Unexpected number '${tens}'!`);
        ...
        if(!(1 <= u_val && u_val <= 9)) throw new RangeError("Ones value must be between 1 and 9 inclusive!");
    ...
    else throw new TypeError(`Invalid number '${str}'!`);
